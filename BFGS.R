...
for (i in 1:1000) {
  ...
  part1 <- 1+((y_k%*%H%*%matrix(y_k))/(y_k %*% matrix(s_k))[1])
  part2 <- (matrix(s_k)%*%s_k)/(y_k %*% matrix(s_k))[1]
  part3 <- (matrix(s_k)%*%y_k%*%H + H%*%matrix(y_k)%*%s_k)/(y_k%*%matrix(s_k))[1]
  delta <- part1[1] * part2 - part3
